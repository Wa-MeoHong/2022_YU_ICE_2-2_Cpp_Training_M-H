...
void Rectang::draw(ConsolePixelFrame cp_frame)
{
	...
	p[0].x = fr_px - width / 2; p[0].y = fr_py - length / 2.0;		// 윗변 왼쪽 좌표
	p[1].x = fr_px + width / 2; p[1].y = fr_py - length / 2.0;		// 윗변 오른쪽 좌표
	p[2].x = fr_px + width / 2; p[2].y = fr_py + length / 2.0;		// 밑변 왼쪽 좌표
	p[3].x = fr_px - width / 2; p[3].y = fr_py + length / 2.0;		// 밑변 오른쪽 좌표
